...
import { CarsAPI, Car, CarStatus, CreateCarDto, UpdateCarDto } from "@/lib/http/cars";
...
export function useCarMutations() {
  const create = React.useCallback(async (body: CreateCarDto) => CarsAPI.create(body), []);
  const update = React.useCallback(async (id: string, body: UpdateCarDto) => CarsAPI.update(id, body), []);
  const remove = React.useCallback(async (id: string) => CarsAPI.remove(id), []);
