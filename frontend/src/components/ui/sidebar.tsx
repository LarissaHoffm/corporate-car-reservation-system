...
function SidebarGroupAction({ className, asChild = false, ...props }: React.ComponentProps<"button"> & { asChild?: boolean; showOnHover?: boolean }) {
...
function SidebarMenuButton({
  ...
  const tooltipProps = typeof tooltip === "string" ? ({ children: tooltip } as React.ComponentProps<typeof TooltipContent>) : tooltip;
...
function SidebarMenuSkeleton({ className, showIcon = false, ...props }: React.ComponentProps<"div"> & { showIcon?: boolean }) {
  ...
  return (
    <div data-slot="sidebar-menu-skeleton" data-sidebar="menu-skeleton" className={cn("flex h-8 items-center gap-2 rounded-md px-2", className)} {...props}>
      ...
      <Skeleton className="h-4 max-w-(--skeleton-width) flex-1" data-sidebar="menu-skeleton-text" style={{ ["--skeleton-width" as any]: width }} />
    </div>
  );
}
